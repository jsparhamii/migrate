...
class discoveryClient(dbclient):
    def get_clusters(self):
      clusters_list = self.get('/clusters/list').get('clusters', [])
      return clusters_list

    def get_num_defined_clusters(self):
      clusters_list = self.clusters_list()
...
client = discoveryClient(token, url)
...
for cluster in clusters: 
  ...
  instance_profile = cluster['aws_attributes'].get('instance_profile_arn', 'No Instance Profile')
  cluster_details.append((cluster_id, cluster_name, creator, node_type, driver_type, custom_tags, spark_version, instance_profile))

# COMMAND ----------

columns = ['cluster_id', 'name', 'creator', 'node_type', 'driver_type', 'custom_tags', 'spark_version', 'instance_profile']
spark.createDataFrame(data=cluster_details, schema = columns).write.mode("overwrite").saveAsTable("uc_discovery.clusters")
